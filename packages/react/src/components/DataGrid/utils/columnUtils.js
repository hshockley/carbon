...
export function getColumnMetrics(metrics) {
  ...
  const unallocatedWidth =
    metrics.viewportWidth - allocatedWidths - getScrollbarSize();
  ...
  const calculatedColumns = columns.map((column, idx) => {
    ...
    const width =
      column.width ??
      clampColumnWidth(unallocatedColumnWidth, column, metrics.minColumnWidth);
    const newColumn = {
      ...column,
      idx,
      width,
